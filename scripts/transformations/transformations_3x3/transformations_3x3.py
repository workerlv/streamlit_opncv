...
def main(image):
    ...
    with translat_col_1:
        txc = st.slider(
            "Translation X combined (tx)",
            -max_width, max_width, 0)
    
    with translat_col_2:
        tyc = st.slider(
            "Translation Y combined (ty)",
    ...
    with skew_rot_col_2:
        rotationc = st.slider(
            "Rotation angle combined (degrees)",
            -90, 90, 45)
        
    combined_image = transform_utils.combined_matrix(image_resized, txc, tyc, rotationc, scalec_x, scalec_y, skewc)
